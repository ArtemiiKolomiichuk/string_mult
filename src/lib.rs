...
pub fn evaluate(input: &str) -> anyhow::Result<String>{
    let data = Grammar::parse(Rule::command, input)?.next().ok_or_else( || anyhow::anyhow!("no command found"))?;
                ...
                match operation{
                    Operation::Mult(ref mut index) => {
                        let index = match index{
                            Some(index) => 
                                if *index < 0 {
                                    (parts.iter().filter(|p| matches!(p, StrPiece::Num(_))).count() as isize + *index) as usize
                                } else {
                                    *index as usize
                                }
                            None => 0                            
